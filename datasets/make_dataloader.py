...
def train_collate_fn(batch):
    ...
    """
    imgs, pids, camids, _ , _ = zip(*batch) #imgs: list of images
    pids = torch.tensor(pids, dtype=torch.int64) #vehicle IDs
    camids = torch.tensor(camids, dtype=torch.int64) #camera IDs
...
def make_dataloader(cfg):
    if cfg.INPUT.RESIZECROP == True: #default false
        randomcrop = T.RandomResizedCrop(cfg.INPUT.SIZE_TRAIN,scale=(0.75,1.0), ratio=(0.75,1.3333), interpolation=3)
    else:
        randomcrop = T.RandomCrop(cfg.INPUT.SIZE_TRAIN) #[384,384]
    train_transforms = T.Compose([
            T.Resize(cfg.INPUT.SIZE_TRAIN, interpolation=3), #[384,384]
            T.RandomHorizontalFlip(p=cfg.INPUT.PROB), #horizontal flip
            T.Pad(cfg.INPUT.PADDING, padding_mode='constant'), #pad 10
            #  T.RandomCrop(cfg.INPUT.SIZE_TRAIN),
            randomcrop, #[384,384]
            T.RandomApply([T.ColorJitter(brightness=0.2, contrast=0.15, saturation=0, hue=0)], p=cfg.INPUT.COLORJIT_PROB),
            T.ToTensor(),
            T.RandomErasing(p=cfg.INPUT.RE_PROB, value=cfg.INPUT.PIXEL_MEAN), #[0.5], [0.485, 0.456, 0.406]
    ...
    num_classes = dataset.num_train_pids #1802, num of vehicle ID 440+1362=1802
    if 'triplet' in cfg.DATALOADER.SAMPLER: # 'softmax_triplet'
        ...
        if cfg.MODEL.DIST_TRAIN: #False
            ...
            train_loader = DataLoader(
                train_set, batch_size=cfg.SOLVER.IMS_PER_BATCH, #64
                sampler=RandomIdentitySampler(dataset.train, cfg.SOLVER.IMS_PER_BATCH, cfg.DATALOADER.NUM_INSTANCE), #bs=64, ins=8
    ...
    if cfg.DATASETS.QUERY_MINING: #false
        val_set = ImageDataset(dataset.query + dataset.query, val_transforms)
    else:
        val_set = ImageDataset(dataset.query + dataset.gallery, val_transforms) #32341
