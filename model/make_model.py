...
class Backbone(nn.Module):
    def __init__(self, num_classes, cfg):
        ...
        last_stride = cfg.MODEL.LAST_STRIDE # 1
        model_path = cfg.MODEL.PRETRAIN_PATH # './pretrained/resnet101_ibn_a-59ea0ac6.pth'
        model_name = cfg.MODEL.NAME # 'resnet101_ibn_a'
        pretrain_choice = cfg.MODEL.PRETRAIN_CHOICE #'imagenet
        self.neck = cfg.MODEL.NECK #bnneck
        self.neck_feat = cfg.TEST.NECK_FEAT #after
        self.model_name = model_name #resnet101_ibn_a'
        ...
        if model_name == 'resnet50':
            ...
            self.in_planes = 2048
            self.base = resnet101_ibn_a(last_stride, frozen_stages=cfg.MODEL.FROZEN) #1, -1
        ...
        if cfg.MODEL.POOLING_METHOD == 'gempoolP':
            ...
            self.gap = nn.AdaptiveAvgPool2d(1) #avg

        self.num_classes = num_classes #1802
        ...
        self.bottleneck.bias.requires_grad_(False) #why requires_grad = False
        self.bottleneck.apply(weights_init_kaiming) #

        if pretrain_choice == 'imagenet' and model_name != 'efficientnet_b7': #default this
    ...
    def forward(self, x, label=None,cam_label=None):  # label is unused if self.cos_layer == 'no'
        if self.model_name =='efficientnet_b7':
            ...
            x = self.base(x)
        global_feat = nn.functional.avg_pool2d(x, x.shape[2:4]) #[64,2048,24,24] -> [64,2048,1,1]
        global_feat = global_feat.view(global_feat.shape[0], -1)  # flatten to (bs, 2048)

        if self.neck == 'no':
            feat = global_feat
        elif self.neck == 'bnneck':
            feat = self.bottleneck(global_feat) #after batch normalization
